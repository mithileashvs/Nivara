...
class ReviewService:
    ...
    async def create(self, patient_id: ObjectId, payload: ReviewCreate) -> dict:
        appt = await self.db[C.APPOINTMENTS].find_one({"_id": oid(payload.appointment_id), "patient_id": patient_id})
        if appt is None:
        ...
        doc = Review(patient_id=patient_id, doctor_id=appt["doctor_id"], appointment_id=appt["_id"], rating=payload.rating, comment=payload.comment).to_mongo()
        try:
    ...
    async def list_mine(self, patient_id: ObjectId, params: PageParams) -> tuple[list[dict], int]:
        return await paginate(self.coll, {"patient_id": patient_id}, params, sort=[("created_at", -1)])

    async def delete(self, review_id: str) -> None:
        doc = await self.coll.find_one_and_delete({"_id": oid(review_id)})
        if doc is None:
