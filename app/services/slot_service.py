...
class SlotService:
    ...
    async def get(self, slot_id: ObjectId) -> dict:
    ...
    async def promote_to_booked(self, slot_id: ObjectId, appointment_id: ObjectId, now: datetime) -> bool:
        doc = await self.slots.find_one_and_update(
    ...
    async def release(self, slot_id: ObjectId, appointment_id: ObjectId) -> dict | None:
    ...
    async def remove_block(self, availability_id: ObjectId) -> int:
        res = await self.slots.update_many(
    ...
    async def delete_window_slots(self, availability_id: ObjectId) -> int:
    ...
    async def block_slot(self, doctor_id: ObjectId, slot_id: ObjectId) -> dict:
        now = utcnow()
        doc = await self.slots.find_one_and_update(
    ...
    async def unblock_slot(self, doctor_id: ObjectId, slot_id: ObjectId) -> dict:
        now = utcnow()
        doc = await self.slots.find_one_and_update(
    ...
    async def _explain_missing(self, doctor_id: ObjectId, slot_id: ObjectId, conflict_msg: str) -> None:
    ...
    async def _present(
        self, slots: list[dict], doctors: dict[ObjectId, dict], now: datetime, detailed: bool = False
    ) -> list[dict]:
